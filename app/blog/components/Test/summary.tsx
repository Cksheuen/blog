...
import type { KbOverviewResponse } from '@/type.d/kb.v1'
...
function PeopleList({
  ...
  return (
    <div>
      <div className="flex items-center justify-center gap-3 rd-t-8 bg-white p-4 pb-0 text-5 font-800 text-amber-9">
        {
          ToSelectedKey.map(item => (
            ...
            >
              <span className={`${item.icon} ${now === item.key
? 'text-4 mx-2'
                : 'text-0'} font-900`}
              >
              </span>
              {item.value}
              {
                now === item.key
                && (
                  <span className="mx-2 align-bottom text-3 line-height-4">
                    共计
                    {people?.count}
                    人
                  </span>
      ...
      <div className="h-100 rd-b-8 bg-white pb-2">
              ...
              <div className={`${style.person} animate-slide-in-left animate-duration-200`} key={manager.id}>
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <div className="mt-1 text-3.5 font-normal">{manager.signature}</div>
                  </div>
                </div>
              </div>
            ))
...
function Header({
  ...
  return (
    <div className={`flex items-center justify-left overflow-hidden mb-10
       ...
       `}
    >
      <div className="m-3 flex items-center justify-left">
        <div className="mr-4 h-30 w-40 flex-shrink-0 overflow-hidden rd-5 bg-cover bg-center !max-w-none">
          <GenerativeBg
            type="terrain"
            content={name}
          />
        </div>
        <div className="flex items-center justify-center line-height-normal">
          <div>
            <div className="inline-block text-5 font-700 text-[--semi-color-text-0]">
              {name}
              <span className="ml-2 text-4 font-1 font-400 text-[--semi-color-text-2] text-shadow-sm text-shadow-color-[--semi-color-text-2]">
                @
                {owner}
              </span>
            </div>
...
export default function Page() {
  ...
  return (
    <div className="text-[--semi-brand-5]">
      ...
      <div className="mx-auto flex items-start justify-center gap-20">
